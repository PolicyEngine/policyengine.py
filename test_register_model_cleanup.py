...
def test_register_model_version_cleans_up_old_data():
    ...
    with db.session as session:
        ...
        initial_bpv_count = len(
            session.exec(
                select(BaselineParameterValueTable).where(
                    BaselineParameterValueTable.model_id
                    == policyengine_uk_model.id
                )
            ).all()
        )

        initial_bv_count = len(
            session.exec(
    ...
    print(
        f"Initial counts - Parameters: {initial_param_count}, BPV: {initial_bpv_count}, BV: {initial_bv_count}"
    )
    ...
    print(
        f"Final counts - Parameters: {final_param_count}, BPV: {final_bpv_count}, BV: {final_bv_count}"
    )
    ...
    print(
        "✅ Test passed - no duplicates when registering model version twice"
    )
