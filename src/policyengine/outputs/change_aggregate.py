...
class ChangeAggregate(BaseModel):
    ...
    def run(self):
        # Get variable object
        var_obj = next(
            v for v in self.baseline_simulation.tax_benefit_model_version.variables
        ...
        with np.errstate(divide='ignore', invalid='ignore'):
            ...
            relative_change_series = relative_change_series.replace([np.inf, -np.inf], np.nan)
        ...
        if self.filter_variable is not None:
            ...
            if filter_var_obj.entity != target_entity:
                filter_mapped = self.baseline_simulation.output_dataset.data.map_to_entity(
