...
class TableLink(BaseModel):
    ...
    def get(self, database: "Database", **kwargs):
        ...
        if self.table_to_model_custom_transforms:
            for field, transform in self.table_to_model_custom_transforms.items():
                if field in model_data:
                    model_data[field] = transform(model_data[field])
                else:
                    model_data[field] = transform(result)

        # Only include fields that exist in the model class
        valid_fields = {field_name for field_name in self.model_cls.__annotations__.keys()}
        filtered_model_data = {k: v for k, v in model_data.items() if k in valid_fields}
    ...
    def set(self, database: "Database", obj: BaseModel):
        ...
        if self.model_to_table_custom_transforms:
            for field, transform in self.model_to_table_custom_transforms.items():
                if field in model_data:
                    model_data[field] = transform(model_data[field])
                else:
                    model_data[field] = transform(obj)
        # Only include fields that exist in the table class
        valid_fields = {field_name for field_name in self.table_cls.__annotations__.keys()}
        filtered_model_data = {k: v for k, v in model_data.items() if k in valid_fields}
        ...
        if isinstance(self.primary_key, tuple):
            for key in self.primary_key:
                query.where(getattr(self.table_cls, key) == getattr(table_obj, key))
        else:
            query.where(getattr(self.table_cls, self.primary_key) == getattr(table_obj, self.primary_key))
