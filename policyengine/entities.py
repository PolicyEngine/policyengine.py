...
class Reform(BaseModel, table=True):
    ...
    is_structural: bool = Field(default=False)  # True if the reform contains non-parametric changes
...
class Parameter(BaseModel, table=True):
    ...
    parameter_changes: List["ParameterChange"] = Relationship(back_populates="parameter")
...
class Entity(BaseModel, table=True):
    ...
    id: Optional[int] = Field(default=None, primary_key=True)
    country_id: int = Field(foreign_key="country.id")
    entity_type: str = Field(index=True)  # Type of entity (person, household, etc.)
    ...
    variable_states: List["VariableState"] = Relationship(back_populates="entity")
...
class VersionedDataset(BaseModel, table=True):
    ...
    datasets: List["Dataset"] = Relationship(back_populates="versioned_dataset")
    dataset_series: "DatasetSeries" = Relationship(back_populates="versioned_datasets")
...
class DatasetSeries(BaseModel, table=True):
    ...
    # Relationships
    versioned_datasets: List["VersionedDataset"] = Relationship(back_populates="dataset_series")


class Dataset(BaseModel, table=True):
    """Tags linking datasets to series with versioning"""
    versioned_dataset_id: int = Field(foreign_key="versioneddataset.id", primary_key=True)
    dataset_series_id: int = Field(foreign_key="datasetseries.id", primary_key=True)
    version: str
...
class Variable(BaseModel, table=True):
    ...
    variable_states: List["VariableState"] = Relationship(back_populates="variable")
...
class VariableState(BaseModel, table=True):
    ...
    time_period: str  # '2025'
    value: str  # '30000'
    simulation_run_id: Optional[int] = Field(default=None, foreign_key="simulationrun.id")
...
def add_uk_sim(
    ...
    person_df = sim.baseline_simulation.calculate_dataframe(["person_id", "age"])
    household_df = sim.baseline_simulation.calculate_dataframe(["household_id", "household_net_income"])
    ...
    with Session(engine) as session:
        ...
        dataset_series = DatasetSeries(
            name="Enhanced FRS",
            description="Enhanced Family Resources Survey"
        )
        ...
        for i in range(len(person_df)):
            ...
            for variable_name in list(person_df.columns):
                ...
                variable = session.exec(select(Variable).where(Variable.name == variable_name).where(Variable.country_id == uk.id)).one()
                variable_state = VariableState(
                    variable=variable,
                    entity=person,
                    time_period="2025",
                    value=str(person_df[variable_name].iloc[i]),  # Convert to string as value is expected to be str
