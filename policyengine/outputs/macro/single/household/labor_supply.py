...
class SingleEconomyLaborSupply(BaseModel):
    substitution_lsr: float = Field(
        default=0, description="Substitution labor supply response"
    )
    income_lsr: float = Field(
        default=0, description="Income labor supply response"
    )
    income_lsr_hh: List[float] = Field(
        default_factory=list,
        description="Household income labor supply response",
    )
    substitution_lsr_hh: List[float] = Field(
        default_factory=list,
        description="Household substitution labor supply response",
    )
    weekly_hours: float = Field(default=0, description="Weekly hours worked")
    weekly_hours_income_effect: float = Field(
        default=0, description="Weekly hours worked income effect"
    )
    weekly_hours_substitution_effect: float = Field(
        default=0, description="Weekly hours substitution effect"
    )
    total_earnings: float = Field(
        ..., description="Total employment and self-employment earnings"
    )
...
def calculate_labor_supply(
    ...
    labor_supply = SingleEconomyLaborSupply(
        ...
        total_workers=int(
            (sim.calculate("employment_income") > 0).sum()
    ...
    if has_behavioral_response(simulation):
        labor_supply.substitution_lsr = sim.calculate(
            "substitution_elasticity_lsr"
        ).sum()
        labor_supply.income_lsr = sim.calculate("income_elasticity_lsr").sum()
        labor_supply.income_lsr_hh = sim.calculate(
            "income_elasticity_lsr", map_to="household"
        ).astype(float)
        labor_supply.substitution_lsr_hh = sim.calculate(
    ...
    if simulation.country == "us":
        ...
        labor_supply.weekly_hours_income_effect = sim.calculate(
            "weekly_hours_worked_behavioural_response_income_elasticity"
        ).sum()
        labor_supply.weekly_hours_substitution_effect = sim.calculate(
