...
class TestBundleTRO:
    ...
    def test__given_preservation_dois__then_performance_records_them(self, monkeypatch):
        monkeypatch.delenv("GITHUB_ACTIONS", raising=False)
        # Synthetic placeholder DOI: exercises propagation mechanics only.
        # Real per-release preservation DOIs are recorded after merge as a
        # per-release op (see #439); no live/stale DOI is embedded here.
        manifest = _us_data_release_manifest().model_copy(
            update={"preservation_dois": ["10.5281/zenodo.9999999"]}
        )
        tro = build_trace_tro_from_release_bundle(
            get_release_manifest("us"),
            manifest,
        ...
        assert performance["pe:preservationDoi"] == ["10.5281/zenodo.9999999"]
