...
def test_baseline_parameter_value_table_set_and_get(fresh_database):
    ...
    model = Model(
        id="bpv_model",
        name="BPV model",
        description="Model for baseline parameter values",
        simulation_function=simulation_function,
    )
    fresh_database.set(model)
    ...
    fresh_database.set(model_version)

    baseline_param = BaselineParameterValue(
        parameter=parameter,
    ...
    fresh_database.set(baseline_param)
    # Note: BaselineParameterValue doesn't have an id field, so we need to query differently
    # For now, we'll skip retrieval test for this model
    # TODO: Add proper retrieval test for composite key models
...
def test_aggregate_table_set_and_get(fresh_database):
    ...
    model = Model(
        id="agg_model",
        name="Agg model",
        description="Model for aggregates",
        simulation_function=simulation_function,
    )
    fresh_database.set(model)
    ...
    fresh_database.set(simulation)

    aggregate = Aggregate(
        simulation=simulation,
        entity="household",
        variable_name="household_income",
        aggregate_function="sum",
        year=2024,
        filter_variable_name="income_positive",
    )

    fresh_database.set(aggregate)
    # Note: Aggregate doesn't have an id field or direct retrieval
    # We'll skip retrieval test for now
    # TODO: Add proper retrieval test for Aggregate model
