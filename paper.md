...
Tax-benefit microsimulation models are essential tools for evaluating the distributional impacts of fiscal policy. Governments, think tanks, and researchers rely on such models to estimate how policy reforms affect household incomes, poverty rates, and government budgets. However, existing models vary widely in scope, access, and reuse. TAXSIM [@taxsim] at NBER computes only tax liabilities and omits the benefit side of the ledger entirely. In the United States, the Congressional Budget Office and the Tax Policy Center maintain microsimulation tax models [@cbo2018taxmodel; @tpc2022taxmodel]. In the UK, UKMOD [@sutherland2014euromod; @ukmod_access] is freely accessible for non-commercial use, but access to its FRS-based input data requires UK Data Service eligibility; HM Treasury uses the Intra-Governmental Tax and Benefit Microsimulation model (IGOTM) [@hmt2025igotm], and the Institute for Fiscal Studies uses TAXBEN [@waters2017taxben].
